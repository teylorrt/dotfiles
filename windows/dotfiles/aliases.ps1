...
# For commands with parameters, the `-p` option is used to indicate that the command requires a parameter. You can also specify a parameter name adding a name after the `-p` option.
## Note: you can variables defined in the environment.ps1 file
# example: 
# setAlias "gis" "git status"
# setAlias "dev" "cd $DEV"
# setAlias "gic" "git checkout" "-p" "branch" -> alias with parameter and parameter name. usage: gic <branch_name>
...
setAlias "gic" "git checkout" "-p" "branch"
setAlias "main" "gic main"
setAlias "master" "gic master"
setAlias "girr" "git reset --hard"
setAlias "giaa" "git add --all"
setAlias "gicm" "gitCommitAndPush" "-p" "message" # gitCommitAndPush is a custom function defined in commands.ps1
